...
class FaceVerificationRegisterScreen extends StatefulWidget {
  final RegistrationData registrationData;

  const FaceVerificationRegisterScreen({
    super.key,
    required this.registrationData,
  });

  @override
  State<FaceVerificationRegisterScreen> createState() =>
      _FaceVerificationRegisterScreenState();
}

class _FaceVerificationRegisterScreenState
    extends State<FaceVerificationRegisterScreen> with WidgetsBindingObserver {
  ...
  Future<void> _compareFaceWithServer(String base64Image) async {
    ...
    try {
      final result = await _authService.compareFaceForRegister(
  ...
  Future<void> _submitRegistration() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() => _isSubmitting = false);
        HapticFeedback.mediumImpact();
        ToastUtils.showError(context, 'Gagal registrasi: ${e.toString()}');
      }
    }
  }
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            Text(
              'Akun Anda telah dibuat. Silakan login dengan username dan password Anda.',
              style: TextStyle(
                fontSize: 14,
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  widget.registrationData.employeeName,
                  style: TextStyle(
                    fontWeight: FontWeight.bold,
                    fontSize: 16,
                    color: isDarkMode
                ...
                Text(
                  'Verifikasi wajah untuk menyelesaikan registrasi',
                  style: TextStyle(
                    fontSize: 12,
                    color: isDarkMode
  ...
  Widget _buildActionButtons(bool isDarkMode) {
    ...
    return Column(
      children: [
        ...
        // Submit Button (only enabled when server says face matches)
        SizedBox(
          width: double.infinity,
          child: ElevatedButton.icon(
            ...
            label: Text(
              _isSubmitting ? 'MENDAFTAR...' : 'SELESAIKAN REGISTRASI',
